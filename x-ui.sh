...
install() {
    bash <(curl -Ls https://ghproxy.com/https://raw.githubusercontent.com/xljl169/scripts/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://ghproxy.com/https://raw.githubusercontent.com/teddysun/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/xljl169/scripts/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
